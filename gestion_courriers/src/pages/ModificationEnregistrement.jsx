...
const ModificationEnregistrement = () => {
  ...
  useEffect(() => {
    const fetchArrive = async () => {
      console.log("Paramètre reçu :", NumeroArrive);
      try {
        const response = await api.get(`/modifier_arrive/${NumeroArrive}`);
        const data = response.data || {};
        console.log(data);
        setModifier({
          DateArrive: data.DateArrive ? data.DateArrive.split("T")[0] : "",
          Provenance: data.Provenance || "",
          numero_correspondance_arrive: data.numero_correspondance_arrive || "",
          DateCorrespondanceArrive: data.DateCorrespondanceArrive ? data.DateCorrespondanceArrive.split("T")[0] : "",
