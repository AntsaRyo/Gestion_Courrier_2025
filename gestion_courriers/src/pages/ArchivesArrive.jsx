...
const ArchivesArrive = () => {
  ...
  const currentPage = "Archives des courriers arrivés";
  ...
  useEffect(() => {
    api.get("/affichageArchiveArrive")
      .then((res) => {
  ...
  const filtre = React.useMemo(() => {
    ...
    const dArr = filterDateArrivee; // format attendu: "YYYY-MM-DD"
    const dCorr = filterDateCorrespondanceArrive;

    return courriers.filter((e) => {
      // sécurité : forcer valeurs en string / safe access
      const provenance = normalize(e.Provenance).toLowerCase();
      const numeroCorr = normalize(e.numero_correspondance_arrive).toLowerCase();
      // corrige l'orthographe si nécessaire : TexteCorrespondanceArrive
      const texte = normalize(e.TexteCorespondanceArrive).toLowerCase();
      ...
      // tests
      if (p && !provenance.includes(p)) return false;
      if (n && !numeroCorr.includes(n)) return false;
      if (t && !texte.includes(t)) return false;
      if (dArr && dateArr !== dArr) return false;
            ...
            {/* DOSSIERS DES DIVISIONS */}
              <div>
                ...
                <ul className="space-y-2 mt-1">
                  ...
                  <li
                    className={`p-2 rounded-md cursor-pointer flex items-center gap-2 font-medium transition ${
                      currentPage === "Archives des courriers arrivés"
                        ? darkMode
        ...
        <main className={`flex-1 p-6 overflow-auto transition-all duration-300 pt-30 relative ${
          ...
          >
            ...
            >
              ...
              >
                Listes des archives des courriers reçus
              </h2>
                ...
                <tbody>
                  {filtre.length === 0 ? (
                    ...
                    filtre.map((item) => (
                      ...
                      >
                        <td className="px-2 py-3 text-center text-sm">{item.NumeroArrive}</td>
                        <td className="px-2 py-3 text-center text-sm">{formatDate(item.DateArrive)}</td>
                        <td className="px-2 py-3 text-center text-sm">{item.Provenance}</td>
                        <td className="px-2 py-3 text-center text-sm">{item.numero_correspondance_arrive}</td>
                        <td className="px-2 py-3 text-center text-sm">{item.piece_jointe_arrive}</td>
                        <td className="px-2 py-3 text-center text-sm">{formatDate(item.DateCorrespondanceArrive)}</td>
                        <td className="px-2 py-3 text-center text-sm">{item.TexteCorespondanceArrive}</td>
                        <td className="px-2 py-3 text-center text-sm">{item.Destinataire}</td>
                        <td className="px-2 py-3 text-center text-sm">{item.observations}</td>
